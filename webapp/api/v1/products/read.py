...
from webapp.crud.product import get_product_by_id, get_products_page
from webapp.db.postgres import get_session
from webapp.logger import logger
from webapp.schema.info.product import ProductInfo, ProductsPageResp
...
async def get_products(
    ...
    try:
        ...
        return ORJSONResponse(
            content=ProductsPageResp.model_validate(serialized_prds).model_dump(), status_code=status.HTTP_200_OK
        )
...
async def get_product(
    ...
    try:
        ...
        return ORJSONResponse(content=ProductInfo.model_validate(product).model_dump(), status_code=status.HTTP_200_OK)
    except HTTPException as http_err:
        raise http_err
    except Exception as e:
        logger.error(f'An error occurred while verifying login information: {e}')
