...
async def get_products_page(session: AsyncSession, offset: int, limit: int, cat_id: int) -> Sequence[Product]:
    return (
        await session.scalars(
            select(Product).where(Product.category_id == cat_id).limit(limit).offset(offset).order_by(Product.id)
        )
    ).all()
...
async def get_product_by_id(session: AsyncSession, id: int):
    return (await session.scalars(select(Product).where(Product.id == id))).one_or_none()
